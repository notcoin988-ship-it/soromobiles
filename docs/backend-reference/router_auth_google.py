...
# Web-клиент мобильного проекта Google Cloud (500782884295). Приложение
# передаёт его как serverClientId, и Google кладёт его в aud выданного
# id_token — именно с этим значением сверяется подпись ниже.
#
# ЭТО НЕ КЛИЕНТ САЙТА. У сайта свой проект (480387520142) и свой web-клиент,
# он используется браузерным OAuth-редиректом /auth/google. Мобильные клиенты
# (Android, iOS) заведены в 500782884295, а Google требует аудиторию из того
# же проекта. Если когда-нибудь оба пути сойдутся в один проект — здесь будет
# одно значение вместо двух.
#
# Не секрет, но берётся из окружения, чтобы стенд и прод не расходились
# правкой кода.
